(function() {
	...
	const options = program
		...
		.option('--pipeopts', 'includes the piped headers in the m3u8 urls')
  ...
  Options:
	...
	const loadConfig = () => {
		const defaultconfig = {
			outdir: '.',
			clientID: 'cf3abce7-f755-4772-ad91-b24ed5834e1a',
			mapping: {
				us: '45.50.96.71'
			},
			all: false,
		...
		try {
			...
			console.error(
				`Got exception while parsing config file ${configfile}`,
				ex.message
			);
			c = {};
		}
